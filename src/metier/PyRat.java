...
public class PyRat {
    ...
    private Map<Point, Boolean> fromageMap;
    ...
    public void preprocessing(Map<Point, List<Point>> laby, int labyWidth, int labyHeight, Point position, List<Point> fromages) {
        fromageMap = new HashMap<>();
        for (Point p : fromages){
            fromageMap.put(p,true);
        }
    }
    ...
    private boolean fromageIci_EnOrdreConstant(Point pos) {
        return fromageMap.containsKey(pos);
    }
    ...
    private void parcoursRecursif(Point n, Map<Point, List<Point>> laby) {
        chemin.add(n);
        for (Point voisin : laby.get(n)) { if (!chemin.contains(voisin)) {  parcoursRecursif(voisin, laby); } }
    }
}